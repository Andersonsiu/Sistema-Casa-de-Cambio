...
def dolares():
    if request.method == 'POST':
        try:
            ...
            if not tasa_cambio or not cantidad_origen or not fecha_transaccion:
...
def euros():
    if request.method == 'POST':
        try:
            ...
            # Formatear la fecha proporcionada por el usuario
            fecha = datetime.strptime(fecha_transaccion, '%Y-%m-%d').strftime('%Y-%m-%d')
...
def reportes():
    if request.method == 'POST':
        try:
            ...
            ganancia_soles_dolares = caja_dolares_compra_origen * diferencia_margen_dolares - gastos
            ganancia_soles_euros = caja_euros_compra_origen * diferencia_margen_euros - gastos

            # Calcular ganancia en dólares
            ...
            # Calcular caja final
            caja_final_dolares = caja_inicial_dolares + ganancia_dolares
            caja_final_euros = caja_inicial_euros + ganancia_euros
                ...
                mongo.db.historico_caja.update_one(
                    ...
                    {'$set': {
                        'caja_dolares': {
                            ...
                            'ganancia_dolares': ganancia_dolares
                        },
                        'caja_euros': {
...
def historico():
    ...
    ganancia_total_dolares = sum(entry['caja_dolares']['ganancia_dolares'] for entry in historico_data)
    ganancia_total_euros = sum(entry['caja_euros']['ganancia_euros'] for entry in historico_data)
    
    return render_template('historico.html', historico_data=historico_data, fechas=fechas, caja_final_dolares=caja_final_dolares, caja_final_euros=caja_final_euros, ganancia_total_dolares=ganancia_total_dolares, ganancia_total_euros=ganancia_total_euros)

@app.route('/exportar_excel')
@login_required
def exportar_excel():
    # Obtener datos del historial desde la base de datos
    historico_data = list(mongo.db.historico_caja.find())
    ...
    # Escribir los encabezados del Excel
    headers = [
        'Fecha', 
        'Caja Inicial Dólares', 'Caja Final Dólares', 'Ganancia en Soles (Dólares)', 'Ganancia en Dólares',
        'Caja Inicial Euros', 'Caja Final Euros', 'Ganancia en Soles (Euros)', 'Ganancia en Euros'
    ]
    for col_num, header in enumerate(headers):
        worksheet.write(0, col_num, header)

    # Escribir los datos del historial en el Excel
    row_num = 1
    for entry in historico_data:
        worksheet.write(row_num, 0, entry['fecha'])
        worksheet.write(row_num, 1, entry['caja_dolares']['inicial'])
        worksheet.write(row_num, 2, entry['caja_dolares']['final'])
        worksheet.write(row_num, 3, entry['caja_dolares']['ganancia_soles'])
        worksheet.write(row_num, 4, entry['caja_dolares']['ganancia_dolares'])
        worksheet.write(row_num, 5, entry['caja_euros']['inicial'])
        worksheet.write(row_num, 6, entry['caja_euros']['final'])
        worksheet.write(row_num, 7, entry['caja_euros']['ganancia_soles'])
        worksheet.write(row_num, 8, entry['caja_euros']['ganancia_euros'])
        row_num += 1
    ...
    return Response(output, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                    headers={"Content-Disposition": "attachment;filename=historial_reportes.xlsx"})
